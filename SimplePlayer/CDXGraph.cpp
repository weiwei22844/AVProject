...
bool CDXGraph::QueryInterfaces(void)
{
	if (mGraph)
	{
        ...
        hr |= mGraph->QueryInterface(IID_IMediaPosition, (void**)&pMediaPosition);
		if (mSeeking)
		{
...
bool CDXGraph::RenderFile(const char * inFile)
{
	...
	if (mGraph)
	{
        ...
        if(pGrabberF)
        {
            hr = mGraph->AddFilter(pGrabberF, L"SimpleGrabber");
            hr = pGrabberF->QueryInterface(IID_ISampleGrabber, (void**)&m_pIGrabber);
            if(SUCCEEDED(hr))
            {
                CMediaType VideoType;
                VideoType.SetType( &MEDIATYPE_Video );
                VideoType.SetSubtype( &MEDIASUBTYPE_RGB24 );
                hr = m_pIGrabber->SetMediaType( &VideoType ); // shouldn't fail
                if( FAILED(hr) )
                {
                    MessageBox(NULL, _T("Init simple grabber failed1"), _T("Error"), MB_OK);
                }
            }
        }
		...
		{
            AM_MEDIA_TYPE mt;
            hr = m_pIGrabber->GetConnectedMediaType( &mt );
            if ( SUCCEEDED( hr) )
            {
                ...
                hr = m_pIGrabber->SetBufferSamples( FALSE );
                
                // only grab one at a time, stop stream after
                // grabbing one sample
                hr = m_pIGrabber->SetOneShot( FALSE );
                
                // set the callback, so we can grab the one sample
                hr = m_pIGrabber->SetCallback( &mCB, 1 );
            }
            else
            {
                MessageBox(NULL, _T("Init simple grabber failed2"), _T("Error"), MB_OK);
            }
			return true;
		}
	}
...
bool CDXGraph::ModifyRate( double rateAdjust )
{
    if(!pMediaPosition)
    {
        return false;
    ...
    hResult = pMediaPosition->get_Rate( &rate );
    if( FAILED(hResult) )
    {
        return false;
    }
    rate += rateAdjust;
    hResult = pMediaPosition->put_Rate( rate );
    if( FAILED(hResult) )
    {
        return false;
    }
...
bool CDXGraph::SetRate( double newRate )
{
    if(!pMediaPosition)
    {
        return false;
    ...
    HRESULT hResult = pMediaPosition->put_Rate( newRate );
    if( FAILED(hResult) )
    {
        return false;
    }
